...
# DevSecOps AI Team — A06 Component Rules Tests
# Tests a06-component-rules.yml structure, metadata,
...
echo "============================================"
echo "DevSecOps AI Team — A06 Component Rules Tests"
echo "============================================"
echo ""

RULES_FILE="$ROOT_DIR/rules/a06-component-rules.yml"
...
[ -f "$RULES_FILE" ] && pass "a06-component-rules.yml exists" || fail "a06-component-rules.yml missing"
...
# ═══════════════════════════════════════════
echo "--- Section 3: OWASP A06 Tags ---"

# All rules tagged with A06:2021
python3 -c "
import yaml, sys
with open('$RULES_FILE') as f:
    data = yaml.safe_load(f)
for rule in data['rules']:
    ...
    if 'A06:2021' not in owasp:
        print(f'{rid} missing A06:2021 tag', file=sys.stderr)
        sys.exit(1)
sys.exit(0)
" && pass "All $RULE_COUNT rules tagged with A06:2021" || fail "Not all rules have A06:2021 tag"

# All rules tagged with A06:2025
python3 -c "
import yaml, sys
with open('$RULES_FILE') as f:
    data = yaml.safe_load(f)
for rule in data['rules']:
    rid = rule.get('id', 'unknown')
    owasp = rule.get('metadata', {}).get('owasp', [])
    if 'A06:2025' not in owasp:
        print(f'{rid} missing A06:2025 tag', file=sys.stderr)
        sys.exit(1)
sys.exit(0)
" && pass "All $RULE_COUNT rules dual-tagged with A06:2025" || fail "Not all rules have A06:2025 tag"
...
for CWE in CWE-1104 CWE-829; do
    python3 -c "
import json, sys
...
# Extract all CWE IDs from rules and verify each exists in mapping
RULE_CWES=$(python3 -c "
import yaml
with open('$RULES_FILE') as f:
    data = yaml.safe_load(f)
...
if '$CWE' in data['mappings']:
    sys.exit(0)
sys.exit(1)
" && pass "$CWE (from rules) found in cwe-to-owasp.json" || fail "$CWE (from rules) not in cwe-to-owasp.json"
done
...
# ═══════════════════════════════════════════
echo "--- Section 6: A06 Fixture Validation ---"

[ -f "$FIXTURE" ] && pass "sample-a06-findings.json exists" || fail "sample-a06-findings.json missing"
...
if len(findings) != 3:
    ...
    sys.exit(1)
for f in findings:
    if not f.get('rule_id', '').startswith('a06-'):
        print(f'{f.get(\"id\",\"?\")} rule_id should start with a06-', file=sys.stderr)
        sys.exit(1)
sys.exit(0)
" && pass "Fixture has 3 findings, all reference a06- rules" || fail "Fixture findings validation failed"
...
echo "============================================"
TOTAL=$((PASS + FAIL))
echo "A06 Rules Tests: $PASS/$TOTAL passed"
