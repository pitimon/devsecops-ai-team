...
# DevSecOps AI Team — A02 Custom Semgrep Rules Tests
...
echo "============================================"
echo "DevSecOps AI Team — A02 Rules Tests"
echo "============================================"
echo ""
...
[ -f "$RULES_FILE" ] && pass "a02-crypto-rules.yml exists" || fail "a02-crypto-rules.yml missing"
...
# All rules tagged with A02:2021
python3 -c "
import yaml, sys
with open('$RULES_FILE') as f:
    data = yaml.safe_load(f)
for rule in data['rules']:
    ...
    if 'A02:2021' not in owasp:
        print(f'{rid} missing A02:2021 tag', file=sys.stderr)
        sys.exit(1)
sys.exit(0)
" && pass "All $RULE_COUNT rules tagged with A02:2021" || fail "Not all rules have A02:2021 tag"
...
# ═══════════════════════════════════════════
echo "--- Section 5: A02 Fixture Validation ---"

[ -f "$FIXTURE" ] && pass "sample-a02-findings.json exists" || fail "sample-a02-findings.json missing"
...
if not isinstance(findings, list) or len(findings) == 0:
    ...
    sys.exit(1)
for f in findings:
    if not f.get('rule_id', '').startswith('a02-'):
        print(f'{f.get(\"id\",\"?\")} rule_id should start with a02-', file=sys.stderr)
        sys.exit(1)
sys.exit(0)
" && pass "All fixture findings reference a02- rules" || fail "Fixture findings validation failed"
...
if command -v docker &>/dev/null && docker info &>/dev/null 2>&1; then
    docker run --rm \
        ...
        semgrep --validate --config /rules/a02-crypto-rules.yml 2>/dev/null \
...
echo "============================================"
TOTAL=$((PASS + FAIL))
echo "A02 Rules Tests: $PASS/$TOTAL passed"
if [ "$FAIL" -gt 0 ]; then
