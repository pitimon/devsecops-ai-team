...
# DevSecOps AI Team — A10 Custom Semgrep Rules Tests
...
echo "============================================"
echo "DevSecOps AI Team — A10 Rules Tests"
echo "============================================"
echo ""

RULES_FILE="$ROOT_DIR/rules/a10-ssrf-rules.yml"
FIXTURE="$ROOT_DIR/tests/fixtures/sample-a10-findings.json"
...
[ -f "$RULES_FILE" ] && pass "a10-ssrf-rules.yml exists" || fail "a10-ssrf-rules.yml missing"
...
")
[ "$RULE_COUNT" -eq 7 ] && pass "Contains 7 rules (got $RULE_COUNT)" || fail "Expected 7 rules, got $RULE_COUNT"
...
EXPECTED_IDS="a10-ssrf-user-url-python a10-ssrf-user-url-js a10-ssrf-user-url-java a10-ssrf-metadata-endpoint a10-ssrf-redirect-follow a10-ssrf-dns-rebinding a10-ssrf-internal-ip"
...
# ═══════════════════════════════════════════
# Section 4: OWASP A10 Tag on All Rules
# ═══════════════════════════════════════════
echo "--- Section 4: OWASP A10 Tag ---"
...
for rule in data['rules']:
    ...
    if 'A10:2021' not in owasp:
        print(f'{rid} missing A10:2021 tag', file=sys.stderr)
        sys.exit(1)
sys.exit(0)
...
expected = {
    'a10-ssrf-user-url-python': 'ERROR',
    'a10-ssrf-user-url-js': 'ERROR',
    'a10-ssrf-user-url-java': 'ERROR',
    'a10-ssrf-metadata-endpoint': 'ERROR',
    'a10-ssrf-redirect-follow': 'WARNING',
    'a10-ssrf-dns-rebinding': 'WARNING',
    'a10-ssrf-internal-ip': 'WARNING'
}
for rid, sev in expected.items():
    ...
    if actual != sev:
        ...
        sys.exit(1)
sys.exit(0)
" && pass "Severity levels correct (ERROR for user-url-*/metadata, WARNING for redirect/dns/internal-ip)" || fail "Severity levels mismatch"
...
# Check CWEs are in NCSA mapping
NCSA_MAPPED=0
for CWE in $RULE_CWES; do
    python3 -c "
import json, sys
...
[ "$NCSA_MAPPED" -gt 0 ] && pass "At least 1 A10 CWE mapped in cwe-to-ncsa.json ($NCSA_MAPPED found)" || fail "No A10 CWEs in NCSA mapping"
...
# ═══════════════════════════════════════════
echo "--- Section 7: A10 Fixture Validation ---"

[ -f "$FIXTURE" ] && pass "sample-a10-findings.json exists" || fail "sample-a10-findings.json missing"
...
if len(findings) != 5:
    ...
    sys.exit(1)
# Each finding should reference A10
for f in findings:
    if f.get('owasp') != 'A10:2021':
        print(f'{f[\"id\"]} missing A10:2021 owasp tag', file=sys.stderr)
        sys.exit(1)
    if not f.get('rule_id', '').startswith('a10-'):
        print(f'{f[\"id\"]} rule_id should start with a10-', file=sys.stderr)
        sys.exit(1)
sys.exit(0)
" && pass "Fixture has 5 findings, all tagged A10:2021" || fail "Fixture findings validation failed"
...
grep -q "a10-ssrf-rules.yml" "$SAST_REF" && pass "sast-patterns.md references A10 rules file" || fail "sast-patterns.md missing A10 rules reference"
...
# ═══════════════════════════════════════════
echo "--- Section 9: Job Dispatcher A10 Support ---"

grep -q "a10" "$DISPATCHER" && pass "job-dispatcher.sh references A10 rules" || fail "job-dispatcher.sh missing A10 support"
...
if command -v docker &>/dev/null && docker info &>/dev/null 2>&1; then
    docker run --rm \
        ...
        semgrep --validate --config /rules/a10-ssrf-rules.yml 2>/dev/null \
