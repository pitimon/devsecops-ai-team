...
# DevSecOps AI Team — A07 Auth Rules Tests
# Tests a07-auth-rules.yml structure, metadata,
...
echo "============================================"
echo "DevSecOps AI Team — A07 Auth Rules Tests"
echo "============================================"
echo ""

RULES_FILE="$ROOT_DIR/rules/a07-auth-rules.yml"
FIXTURE="$ROOT_DIR/tests/fixtures/sample-a07-findings.json"
...
[ -f "$RULES_FILE" ] && pass "a07-auth-rules.yml exists" || fail "a07-auth-rules.yml missing"
...
# ═══════════════════════════════════════════
echo "--- Section 3: OWASP A07 Tags ---"

# All rules tagged with A07:2021
python3 -c "
import yaml, sys
with open('$RULES_FILE') as f:
    data = yaml.safe_load(f)
for rule in data['rules']:
    ...
    if 'A07:2021' not in owasp:
        print(f'{rid} missing A07:2021 tag', file=sys.stderr)
        sys.exit(1)
sys.exit(0)
" && pass "All $RULE_COUNT rules tagged with A07:2021" || fail "Not all rules have A07:2021 tag"

# All rules tagged with A07:2025
python3 -c "
import yaml, sys
with open('$RULES_FILE') as f:
    data = yaml.safe_load(f)
for rule in data['rules']:
    rid = rule.get('id', 'unknown')
    owasp = rule.get('metadata', {}).get('owasp', [])
    if 'A07:2025' not in owasp:
        print(f'{rid} missing A07:2025 tag', file=sys.stderr)
        sys.exit(1)
sys.exit(0)
" && pass "All $RULE_COUNT rules dual-tagged with A07:2025" || fail "Not all rules have A07:2025 tag"
...
for CWE in CWE-521 CWE-798 CWE-613 CWE-308 CWE-640; do
    python3 -c "
import json, sys
...
print(len(all_langs))
")
[ "$LANG_COUNT" -ge 1 ] && pass "At least 1 language covered across all rules ($LANG_COUNT found)" || fail "No languages covered ($LANG_COUNT found)"
...
# ═══════════════════════════════════════════
echo "--- Section 6: A07 Fixture Validation ---"

[ -f "$FIXTURE" ] && pass "sample-a07-findings.json exists" || fail "sample-a07-findings.json missing"
...
if len(findings) != 3:
    ...
    sys.exit(1)
for f in findings:
    if not f.get('rule_id', '').startswith('a07-'):
        print(f'{f.get(\"id\",\"?\")} rule_id should start with a07-', file=sys.stderr)
        sys.exit(1)
sys.exit(0)
" && pass "Fixture has 3 findings, all reference a07- rules" || fail "Fixture findings validation failed"
...
echo "============================================"
TOTAL=$((PASS + FAIL))
echo "A07 Rules Tests: $PASS/$TOTAL passed"
